...
local fcoglUIguildMembersJoinedListWindow
...
fcoglUI.ListTypeToListObject = {
    ...
    [FCOGL_LISTTYPE_GUILD_MEMBERS_JOIN_DATE] =  fcoglUIguildMembersJoinedListWindow,
...
local function hideLeftTLCListControlsExceptThis(doNotHideThisListObject)
    df("hideLeftTLCListControlsExceptThis - doNotHideObject: %s", tos(doNotHideThisListObject))
    --Hide all lists
...
------------------------------------------------------------------------------------------------------------------------
        ...
        if listType == FCOGL_LISTTYPE_ROLLED_DICE_HISTORY then
            ...
            elseif guildMemberJoinedListActive == true then
                ...
                tableWithLastDiceThrows = FCOGuildLottery.diceRollGuildMemberJoinedListHistory[currentlyUsedGuildMembersJoinDateGuildId][currentlyUsedGuildMembersJoinDateUniqueId][currentlyUsedGuildMembersJoinDateTimestamp]
...
function fcoglWindowClass:InitializeComboBox(control, prefix, max, exclude, searchBoxType )
    ...
    df("[fcoglWindowClass:InitializeComboBox]isGuildLotteryHistoryCB: %s, isGuildsCB: %s, isNameSearchCB: %s, prefix: %s, max: %s", tos(isGuildLotteryHistoryCB), tos(isGuildsCB), tos(isNameSearchCB), tos(prefix), tos(max))
        ...
        if guildSalesLotteryHistoriesSaved ~= nil then
            ...
            for timeStampOfGuildSalesLotteryHistoryEntry, guildSalesLotteryHistoryEntryData in pairs(guildSalesLotteryHistoriesSaved) do
                if timeStampOfGuildSalesLotteryHistoryEntry ~= "daysBefore" then
                    ...
                    local dateTimeString = string.format(FCOGuildLottery.FormatDate(timeStampOfGuildSalesLotteryHistoryEntry) .. " (#%s)", tos(countDiceThrowData))
                                ...
                                if not menuItemsPostHooked[itemInMenu] then
                                    ZO_PostHookHandler(itemInMenu, "OnMouseUp", function(entryControl, mouseButton, isUpInside)
                                        if isUpInside and mouseButton == MOUSE_BUTTON_INDEX_RIGHT then
                                            ...
                                            AddCustomMenuItem(string.format("Delete dice history entry\'%s\'", tos(entryData.name)), function()
...
function fcoglUI.createWindow()
    ...
    if fcoglUIwindow ~= nil and fcoglUIwindowFrame ~= nil then
        ...
        if not FCOGuildLottery.UI.guildMembersJoinedListWindow then
            --Guild members joined list
            FCOGuildLottery.UI.guildMembersJoinedListWindow = fcoglWindowClass:New(FCOGLFrameMembersJoined, FCOGL_LISTTYPE_GUILD_MEMBERS_JOIN_DATE, fcoglUIwindowFrame)
            fcoglUIguildMembersJoinedListWindow = FCOGuildLottery.UI.guildMembersJoinedListWindow
        end
...
df("showUIWindow: " ..tos(doShow))
    ...
    if doShow == nil then
        ...
        if doShow == true then
            ...
            --Guild sales lottery           fcoglUIguildSalesLotteryWindow
            --Guild members joined list     fcoglUIguildMembersJoinedListWindow
            --todo 20221218 if listType is FCOGL_LISTTYPE_NORMAL_THROWS or FCOGL_LISTTYPE_GUILD_MEMBER_THROWS: What list should be shown then? None?
            local currentlyShownListObject = getCurrentlyShownListsObject()
            if currentlyShownListObject == nil and FCOGuildLottery.UI.firstCallOfShowUIWindow == true then
                ...
                currentlyShownListObject = fcoglUIguildSalesLotteryWindow
            end
...
df(">listType: %s, sortKey: %s, sortOrder: %s", tos(FCOGL_LISTTYPE_GUILD_SALES_LOTTERY), tos(fcoglUIwindow.currentSortKey), tos(fcoglUIwindow.currentSortOrder))
    ...
    if fcoglUIguildMembersJoinedListWindow ~= nil then
        ...
        fcoglUIguildMembersJoinedListWindow.currentSortOrder = fcoglUIguildMembersJoinedListWindow.sortHeaderGroup:GetSortDirection()

        FCOGuildLottery.settingsVars.settings.UIwindow.sortKeys[currentTab][FCOGL_LISTTYPE_GUILD_MEMBERS_JOIN_DATE]  = fcoglUIguildMembersJoinedListWindow.currentSortKey
        FCOGuildLottery.settingsVars.settings.UIwindow.sortOrder[currentTab][FCOGL_LISTTYPE_GUILD_MEMBERS_JOIN_DATE] = fcoglUIguildMembersJoinedListWindow.currentSortOrder
df(">listType: %s, sortKey: %s, sortOrder: %s", tos(FCOGL_LISTTYPE_GUILD_MEMBERS_JOIN_DATE), tos(fcoglUIguildMembersJoinedListWindow.currentSortKey), tos(fcoglUIguildMembersJoinedListWindow.currentSortOrder))
    end
    ...
    if alsoDeleteSV == true then
        ...
        if FCOGuildLottery.IsGuildSalesLotteryActive() then
            ...
            if entryData ~= nil and entryData.timestamp ~= nil then
                df(">entryData switched the timestamp to: %s", tos(entryData.timestamp))
            ...
            if guildId ~= nil then
                if FCOGuildLottery.settingsVars.settings.diceRollGuildsHistory[guildId] ~= nil then
                    ...
                    df("Guild history data deleted, guildId %s", tos(guildId))
                    wasDeleted = true
        ...
        if wasDeleted == true then
            dfa(string.format(GetString(FCOGL_CLEARED_HISTORY_COUNT), tos(countDeletedItems)))
        end
    end
    ...
    if guildSalesLotteryIsActive == true then
        ...
        if FCOGuildLottery.diceRollGuildLotteryHistory[currentGuildSalesLotteryGuildId] and
            ...
            for timeStamp, dataOfGuildSalesLotteryRolls in pairs(currentGuildSalesLotteryHistoryEntries) do
                ...
                local dataEntry = {}
                local dateTimeString = string.format(FCOGuildLottery.FormatDate(timeStamp) .. " (#%s)", tos(countDiceThrowData))
...
function fcoglUI.checkDeleteSelectedGuildSalesLotteryHistoryEntries()
    ...
    FCOGuildLottery.showAskDialogNow(nil, nil, false,
        ...
        function()  end,
        {
            ...
            question    = string.format(GetString(FCOGL_DELETE_HISTORY_ENTRIES_DIALOG_QUESTION), tos(numSelectedEntries)),
        },
        true
    )
end
